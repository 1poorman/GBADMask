...
上表是**单次运行**结果。后续用同配置（MobileViG + `ATTN=gc`）重跑一次作骨干基线时，
得到 segm AP **5.084**，与表中 `gc` 组的 6.797 相差 **1.7 AP**。

为定位噪声来源做了对照：用**同一个已训练模型**评估两次得 6.833 与 6.797（差 0.036）
—— 说明**评估是确定的，噪声全部来自训练过程**。

根因：`tools/base.py` 曾无条件设置 `torch.backends.cudnn.benchmark = True`。
该选项会为每种输入尺寸在运行时探测最快卷积算法：

1. 小麦配置用**多尺度训练**（`MIN_SIZE_TRAIN` 有 6 档），尺寸并不固定，
   每次遇到新尺寸都要重新探测，反而更慢；
2. 探测结果依赖运行时状态，使训练**不可复现**。

已修复：删除无条件设置，改由 `cfg.CUDNN_BENCHMARK`（默认 `False`）在各脚本
`setup()` 中统一控制；需要极致速度时可加 `CUDNN_BENCHMARK True` 覆盖。

**对各结论的影响**（噪声约 ±1.7 AP）：

| 结论 | 差异 | 是否可靠 |
| --- | --- | --- |
| FDC 语义损失有害（6.797 → 0.611） | −6.19 | ✅ 远超噪声，**可靠** |
| ProtoNetV2 优于官方（gc vs base） | +1.64 | ❌ 在噪声内，**待多 seed 验证** |
| GC 净贡献（gc vs nogc） | +1.26 | ❌ 在噪声内 |
| 通道 > 空间注意力（gc vs spatial） | +1.40 | ❌ 在噪声内 |
| 位置编码无增益（gc vs coord） | +1.47 | ❌ 在噪声内 |

因此 3.7 节中**只有"FDC 有害"可以采信**，其余结论需按第 9 节建议做多 seed 重跑。
数据仍完整列出，作为排查过程的记录。

### 3.7.1 多 seed 复验的最终结论（23 组，已完成）

针对上述噪声问题，后续共跑了 **23 组训练**（base 5 次 / gc 4 次 / c2f 3 次 +
各组件），统一 4000 iter。**完整数据与统计检验见 `ABLATION_RESULTS.md`**，要点：

| 结论 | 数据 | 强度 |
| --- | --- | --- |
| **语义辅助损失有害**（任何实现 × 梯度截断） | fdc 0.611 / ft+detach 1.012 / ce+detach 0.629 vs base 5.106 | ✅ 差 −4.1~−4.5，唯一可靠结论 |
| ProtoNetV2 ≈ 官方 ProtoNet | base 5.106±0.40 (n=5) vs gc 5.498±0.89 (n=4)，t≈0.81，p>0.4 | ⚠️ 无显著差异 |
| LSK 无效果 | lskon 5.630 vs lskoff 5.656（同批次） | ✅ 较可靠 |
| C2f 改造无收益 | c2f 5.117±0.74 (n=3) vs base 5.106 | ⚠️ 无差异 |
| ECA（BiFPN 内）负向迹象 | 4.700 vs 同批次 vig 5.084 | ⚠️ 单次 |
| coord / cbam / spatial / c3k5 / c3k7 | 5.3~5.9 | ❌ 噪声内，不作结论 |

**同 seed 跨批次噪声仍达 0.1~2.0 AP**（GPU 原子操作非确定性所致，
cudnn.benchmark 修复只能消除其中一部分）。

**对论文/报告的诚实表述建议**：
在 584 图规模下，ProtoNetV2 的全部架构改动无法与训练噪声区分；
唯一有价值的发现是**负结果**——语义辅助损失在此类前景稀少的数据上
（背景 >95%，单图仅 1-2 类）必然有害，且与损失形式、梯度截断无关，
这与官方默认 `LOSS_ON=False` 相互印证。
