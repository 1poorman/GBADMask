...
| B1（窗口 PSA 双点） | OOM | — | batch 7 显存 15487M 超预算 → 收尾队列 B1'' 仅 tower（ATTN_LOW=none，省 ~0.8GB） |
| C1（NUM_BASES 8） | OOM | — | 放弃（显存放不下且预期收益弱） |

**组件池净结论**：C3K2（骨干侧，+1.1）已锁定；EMA（+0.40）候选；PSA 待 B1''；
其余淘汰。距 +5 AP 目标的缺口由 R1 锚点（收尾队列重跑中）定义。
