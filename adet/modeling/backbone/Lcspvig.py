...
class BN_LSKb_act(nn.Module):
    ...
    def __init__(self, in_channels: object, out_channels: object, kernel_size: object, stride: object, padding: object,
                 dilation=1, groups=1, bias=False) -> object:
        super(BN_LSKb_act, self).__init__()
        self.seq = nn.Sequential(
           nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride,
                      padding=padding, dilation=dilation, groups=groups, bias=bias),
            LSKblock(out_channels),
            nn.BatchNorm2d(out_channels),
            nn.Hardswish()  # nn.Mish, nn.GELU
           # nn.Mish()
        )
...
def build_Lcspvigm_backbone(cfg, input_shape):
...
@BACKBONE_REGISTRY.register()
def build_Lcspvigm_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    """
...
@BACKBONE_REGISTRY.register()
def build_Lcspvigb_backbone(cfg, input_shape): 
